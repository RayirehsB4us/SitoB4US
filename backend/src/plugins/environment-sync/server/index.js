...
function crossFetch(url, method, body, token) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.on('timeout', () => { req.destroy(); reject(new Error('Request to master timed out')); });
    req.write(data);
    req.end();
  });
}
      ...
      if (fs.existsSync(ctDir)) {
        for (const ctName of fs.readdirSync(ctDir)) {
          ...
          if (fs.existsSync(schemaPath)) {
            try { diskSchemas[`${apiName}.${ctName}`] = JSON.parse(fs.readFileSync(schemaPath, 'utf8')); } catch (_) {}
          }
        }
      }
    }
...
module.exports = {
  ...
  controllers: {
    ...
    git: {
      ...
      async commit(ctx) {
        ...
        if (!message) { ctx.status = 400; ctx.body = { error: 'Commit message is required' }; return; }
        try {
          await execAsync('git add -A', { cwd: REPO_ROOT });
          const { stdout } = await execAsync(`git commit -m ${JSON.stringify(message)}`, { cwd: REPO_ROOT });
      ...
      async push(ctx) {
        try {
          ...
          if (!token) { ctx.status = 400; ctx.body = { error: 'GitHub token not configured. Save your token first.' }; return; }
          const { branch } = ctx.request.body;
          if (!branch) { ctx.status = 400; ctx.body = { error: 'No branch selected.' }; return; }
          ...
          const { stdout, stderr } = await execAsync(`git push ${authenticatedUrl} ${branch}`, { cwd: REPO_ROOT });
      ...
      async getSettings(ctx) {
        try {
          ...
          ctx.body = {
            hasToken: !!token,
            tokenMasked: token ? `${token.slice(0, 4)}${'•'.repeat(Math.max(0, token.length - 8))}${token.slice(-4)}` : null,
          };
        } catch (err) {
      ...
      async saveToken(ctx) {
        ...
        if (!token) { ctx.status = 400; ctx.body = { error: 'Token is required' }; return; }
        const store = strapi.store({ type: 'plugin', name: 'environment-sync' });
    ...
    backup: {
        ...
        try {
          ...
          const files = fs.readdirSync(BACKUPS_DIR)
            ...
            .map(filename => {
              ...
              return { filename, size: formatBytes(stat.size), sizeBytes: stat.size, createdAt: stat.mtime.toISOString(), ago: timeAgo(stat.mtime) };
            })
      ...
      async create(ctx) {
        try {
          ...
          for (const [uid, comp] of Object.entries(allComponents)) {
            components[uid] = {
              uid: comp.uid, category: comp.category,
              displayName: comp.info && comp.info.displayName,
              attributes: comp.attributes, options: comp.options,
            };
          }

          // Data for api:: types + optionally upload.file
          const dataUids = Object.keys(allContentTypes).filter(uid =>
            uid.startsWith('api::') || (!excludeMedia && uid === 'plugin::upload.file')
          );
          const data = {};
          for (const uid of dataUids) {
            try {
              data[uid] = await strapi.entityService.findMany(uid, { populate: '*', pagination: { limit: -1 } });
            } catch (e) {
          ...
          const payload = {
            meta: {
              ...
              environment: process.env.NODE_ENV || 'development', excludeMedia,
              totalContentTypes: Object.keys(schemas).length,
              totalComponents: Object.keys(components).length, totalEntries,
            },
            schemas, components, diskSchemas, diskComponents, data,
          };
          ...
          ctx.body = {
            success: true, filename, size: formatBytes(compressed.length),
            totalEntries, totalContentTypes: Object.keys(schemas).length, totalComponents: Object.keys(components).length,
          };
        } catch (err) {
          ctx.status = 500;
      ...
      async download(ctx) {
        ...
        if (!filename || filename.includes('/') || filename.includes('..') || !filename.endsWith('.json.gz')) {
          ctx.status = 400; ctx.body = { error: 'Invalid filename' }; return;
        }
        const filePath = path.join(BACKUPS_DIR, filename);
        if (!fs.existsSync(filePath)) { ctx.status = 404; ctx.body = { error: 'Backup file not found' }; return; }
    ...
    sync: {
        ...
        try {
          ...
          ctx.body = {
            ...
            tokenMasked: config.transferToken
              ? `${config.transferToken.slice(0, 4)}${'•'.repeat(Math.max(0, config.transferToken.length - 8))}${config.transferToken.slice(-4)}`
              : null,
          };
        } catch (err) {
          ctx.body = { role: 'master', masterUrl: '', hasToken: false, tokenMasked: null };
        }
      },
      ...
      async saveConfig(ctx) {
        ...
        try {
          ...
          const updated = {
            ...
            masterUrl: masterUrl !== undefined ? masterUrl : (existing.masterUrl || ''),
      ...
      async sendRequest(ctx) {
        ...
        try {
          ...
          const config = await store.get({ key: 'sync_config' }) || {};
          if (!config.masterUrl) {
            ctx.status = 400; ctx.body = { error: 'Master URL not configured. Save your Sync Config first.' }; return;
          }
      ...
      async receiveRequest(ctx) {
        ...
        try {
          ...
          if (!config.transferToken || token !== config.transferToken) {
            ctx.status = 401; ctx.body = { error: 'Invalid transfer token' }; return;
          }
          ...
          await store.set({ key: 'sync_requests', value: requests.slice(0, 50) });
          ctx.body = { success: true };
        } catch (err) {
          ctx.status = 500;
          ctx.body = { error: err.message };
      ...
      async status(ctx) {
        ...
        try {
          ...
          if (!request) { ctx.status = 404; ctx.body = { error: 'Request not found' }; return; }
      ...
      async approve(ctx) {
        ...
        try {
          ...
          if (request.status !== 'pending') {
            ctx.status = 400; ctx.body = { error: `Request is already ${request.status}` }; return;
          }
          ...
          if (syncType === 'data' || syncType === 'both') {
            const data = {};
            const dataUids = Object.keys(strapi.contentTypes).filter(uid => uid.startsWith('api::'));
            for (const uid of dataUids) {
              try {
                data[uid] = await strapi.entityService.findMany(uid, { populate: '*', pagination: { limit: -1 } });
              } catch (e) {
          ...
          fs.writeFileSync(path.join(TRANSFERS_DIR, `transfer_${safeId}.json.gz`), await gzip(JSON.stringify(payload)));

          await store.set({
            key: 'sync_requests',
            value: requests.map(r => r.id === id ? { ...r, status: 'approved', approvedAt: new Date().toISOString() } : r),
          });
      ...
      async reject(ctx) {
        ...
        try {
          const store = strapi.store({ type: 'plugin', name: 'environment-sync' });
          const requests = await store.get({ key: 'sync_requests' }) || [];
          if (!requests.find(r => r.id === id)) { ctx.status = 404; ctx.body = { error: 'Request not found' }; return; }
          await store.set({
            key: 'sync_requests',
            value: requests.map(r => r.id === id ? { ...r, status: 'rejected', rejectedAt: new Date().toISOString() } : r),
          });
          ctx.body = { success: true };
        } catch (err) {
          ctx.status = 500;
      ...
      async transfer(ctx) {
        ...
        try {
          ...
          if (!request || request.status !== 'approved') {
            ctx.status = 403; ctx.body = { error: 'Transfer not approved' }; return;
          }
          const safeId = id.replace(/[^a-zA-Z0-9_-]/g, '');
          const transferFile = path.join(TRANSFERS_DIR, `transfer_${safeId}.json.gz`);
          if (!fs.existsSync(transferFile)) {
            ctx.status = 404; ctx.body = { error: 'Transfer file not found on master' }; return;
          }
          const payload = JSON.parse((await gunzip(fs.readFileSync(transferFile))).toString());
      ...
      async apply(ctx) {
        ...
        if (!requestId) { ctx.status = 400; ctx.body = { error: 'requestId is required' }; return; }
        try {
          const store = strapi.store({ type: 'plugin', name: 'environment-sync' });
          const config = await store.get({ key: 'sync_config' }) || {};
          if (!config.masterUrl) { ctx.status = 400; ctx.body = { error: 'Master URL not configured' }; return; }
          ...
          if (payload.diskSchemas) {
            ...
            for (const [key, schema] of Object.entries(payload.diskSchemas)) {
              ...
              const schemaPath = path.join(process.cwd(), 'src', 'api', apiName, 'content-types', ctName, 'schema.json');
              try {
            ...
            if (payload.diskComponents) {
              for (const [key, schema] of Object.entries(payload.diskComponents)) {
                const dot = key.indexOf('.');
            ...
            results.schemas = { schemaFiles: schemaCount, componentFiles: compCount, requiresRestart: schemaCount > 0 };
          }
          ...
          if (payload.data) {
            ...
            for (const [uid, entries] of Object.entries(payload.data)) {
              ...
              try {
                const existing = await strapi.entityService.findMany(uid, { fields: ['id'], pagination: { limit: -1 } });
                for (const e of existing) await strapi.entityService.delete(uid, e.id);
                for (const entry of entries) {
                  const { id: _id, createdAt: _ca, updatedAt: _ua, ...data } = entry;
  ...
  routes: {
    admin: {
      ...
      routes: [
        // Git
        { method: 'GET',  path: '/git/status',    handler: 'git.status',      config: { policies: [], auth: false } },
        { method: 'GET',  path: '/git/branches',  handler: 'git.branches',    config: { policies: [], auth: false } },
        { method: 'POST', path: '/git/commit',     handler: 'git.commit',      config: { policies: [], auth: false } },
        { method: 'POST', path: '/git/push',       handler: 'git.push',        config: { policies: [], auth: false } },
        { method: 'GET',  path: '/settings',       handler: 'git.getSettings', config: { policies: [], auth: false } },
        { method: 'POST', path: '/settings/token', handler: 'git.saveToken',   config: { policies: [], auth: false } },
        // Backup
        { method: 'GET',    path: '/backup/list',               handler: 'backup.list',     config: { policies: [], auth: false } },
        { method: 'POST',   path: '/backup/create',             handler: 'backup.create',   config: { policies: [], auth: false } },
        { method: 'GET',    path: '/backup/download/:filename', handler: 'backup.download', config: { policies: [], auth: false } },
        { method: 'DELETE', path: '/backup/:filename',          handler: 'backup.remove',   config: { policies: [], auth: false } },
        ...
        { method: 'POST', path: '/sync/requests/:id/reject',  handler: 'sync.reject',       config: { policies: [], auth: false } },
      ],
    },
  },
};
