...
var COLLECTION_PAGE_SIZE = 1; // esporta una sola entry per richiesta
var SLEEP_BETWEEN_REQUESTS_MS = 500; // pausa tra le chiamate HTTP per non stressare Strapi

// ─── Tracker file media copiati ──────────────────────────────────
var copiedMediaFiles = [];

// ─── Utility: sleep non bloccante ────────────────────────────────
function sleep(ms) {
...
// ─── Fetch Single Type con populate "a fette" (per alleggerire Strapi) ────────
async function fetchSingleTypeChunked(st) {
  if (!Array.isArray(st.populate) || st.populate.length === 0) {
    var url = STRAPI_URL + '/' + st.apiId + '?populate=*';
    var resp = await axios.get(url);
    return resp.data && resp.data.data ? (resp.data.data.attributes || resp.data.data) : null;
  }

  var mergedAttrs = null;

  for (var i = 0; i < st.populate.length; i++) {
    var comp = st.populate[i];
    var urlComp = STRAPI_URL +
      '/' + st.apiId +
      '?populate[' + comp + '][populate]=*';

    var respComp = await axios.get(urlComp);
    if (!respComp.data || !respComp.data.data) {
      continue;
    }

    var attrsComp = respComp.data.data.attributes || respComp.data.data;

    if (!mergedAttrs) {
      mergedAttrs = attrsComp;
    } else {
      Object.keys(attrsComp).forEach(function(key) {
        if (key === comp) {
          mergedAttrs[key] = attrsComp[key];
        }
      });
    }
  }
...
async function exportSingleTypes() {
  ...
  for (var i = 0; i < config.SINGLE_TYPES.length; i++) {
    ...
    try {
      ...
      // Per i tipi più pesanti (es. organizzazione) usa le richieste chunked
      if (st.apiId === 'organizzazione') {
        attrs = await fetchSingleTypeChunked(st);
      } else {
        var queryStr = config.buildPopulateQuery(st.populate);
        var url = STRAPI_URL + '/' + st.apiId + queryStr;
        var response = await axios.get(url);
        attrs = response.data && response.data.data
...
async function exportCollectionTypes() {
  ...
  for (var i = 0; i < config.COLLECTION_TYPES.length; i++) {
    ...
    try {
      while (true) {
        var url = STRAPI_URL + '/' + ct.pluralId +
                  '?populate=*&pagination[pageSize]=' + COLLECTION_PAGE_SIZE +
        ...
        var pageItems = data.map(function(item) {
          return cleanAttributes(item.attributes || item);
        });

        allItems = allItems.concat(pageItems);

        var pagination = response.data.meta && response.data.meta.pagination
      ...
      if (allItems.length > 0) {
        result[ct.pluralId] = allItems;
        console.log('  ✅ ' + ct.displayName + ' (' + allItems.length + ' elementi)');
      } else {
